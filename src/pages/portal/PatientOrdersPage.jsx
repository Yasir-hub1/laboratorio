...
function orderDetailPath(order) {
  ...
  return ROUTES.PATIENT_ORDER_DETAIL.replace(':orderId', String(id))
}
    ...
    <div className="glass-card animate-pulse rounded-2xl border p-4 sm:p-5">
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1 space-y-2.5">
          ...
          <div className="h-3 w-1/2 max-w-[9rem] rounded-md bg-surface-muted/80" />
          <div className="h-3 w-3/5 max-w-[10rem] rounded-md bg-surface-muted/70" />
        </div>
      </div>
    </div>
  )
...
function OrderCard({ order, disabled }) {
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        'hover:border-emerald-400/35 hover:shadow-[0_12px_40px_-16px_rgba(34,197,94,0.25)]',
        'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40',
        disabled && 'pointer-events-none opacity-60',
      )}
    >
      <div className="absolute inset-y-0 right-0 w-1 bg-gradient-to-b from-emerald-400/0 via-emerald-400/40 to-emerald-400/0 opacity-0 transition-opacity group-hover:opacity-100" />

      <div className="flex items-start gap-3 sm:gap-4">
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-600 ring-1 ring-emerald-500/15 sm:h-12 sm:w-12">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-2 flex flex-col gap-1.5 text-sm text-muted">
                <span className="inline-flex items-center gap-2">
                  <Calendar className="h-3.5 w-3.5 shrink-0 text-emerald-600/80" aria-hidden />
                  <span>{orderDate !== '—' ? orderDate : 'Sin fecha de orden'}</span>
                </span>
                {branchName && (
                  <span className="inline-flex items-center gap-2">
                    <Building2 className="h-3.5 w-3.5 shrink-0 text-emerald-600/80" aria-hidden />
        ...
        <ChevronRight
          className="mt-1 h-5 w-5 shrink-0 text-muted/50 transition-transform group-hover:translate-x-0.5 group-hover:text-emerald-600"
          aria-hidden
        />
      </div>
    </Link>
    ...
    <AnimatedPage className="pb-24 sm:pb-8">
      ...
      <section className="glass-card mb-4 overflow-hidden rounded-2xl border p-4 sm:mb-6 sm:p-5">
        <div className="flex items-start gap-3 sm:items-center">
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-emerald-500/15 text-emerald-600 ring-1 ring-emerald-500/20">
          ...
          <div className="min-w-0 flex-1">
            ...
            {patientId && (
              <p className="mt-1 truncate text-sm text-muted">{patientId}</p>
            )}
          </div>
          {!loading && totalRows > 0 && (
        ...
        <div className="space-y-3">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <Button
              ...
              aria-controls="patient-order-filters"
            >
      ...
      <section aria-live="polite" aria-busy={loading}>
        ...
        {showNoResults && !hasFilters && isEmpty && (
          <Card className="px-4 py-10 text-center sm:px-8 sm:py-12">
            ...
            <p className="text-base font-semibold text-foreground">No tienes órdenes completadas</p>
            <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-muted">
              Cuando el laboratorio complete una orden, aparecerá aquí para consultar resultados.
            </p>
          </Card>
        )}
          ...
          <>
            ...
            <ul className="space-y-3 md:grid md:grid-cols-2 md:gap-4 md:space-y-0 xl:grid-cols-1 xl:space-y-3">
              {items.map((order, index) => (
                ...
                >
                  <OrderCard order={order} disabled={isLoading} />
                </motion.li>
              ))}
            </ul>
          </>
      ...
      {items.length > 0 && totalRows > 0 && (
        <div
          className={cn(
            ...
            'fixed inset-x-0 bottom-0 z-20 mx-auto max-w-4xl px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 sm:relative sm:inset-auto sm:mx-0 sm:max-w-none sm:pb-0',
